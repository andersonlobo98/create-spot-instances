...
VOLUME_ID=$(aws ec2 describe-volumes --region $REGION --filters "Name=tag:Persistence,Values=true" "Name=tag:Environment,Values=${ENVIRONMENT}" "Name=tag:InstanceIndex,Values=$INSTANCE_INDEX" "Name=availability-zone,Values=$AVAILABILITY_ZONE" "Name=status,Values=available" --query "Volumes[0].VolumeId" --output text)

# Se não houver um volume específico, obter qualquer volume disponível
if [ "$VOLUME_ID" == "None" ] || [ -z "$VOLUME_ID" ]; then
  VOLUME_ID=$(aws ec2 describe-volumes --region $REGION --filters "Name=tag:Persistence,Values=true" "Name=tag:Environment,Values=${ENVIRONMENT}" "Name=availability-zone,Values=$AVAILABILITY_ZONE" "Name=status,Values=available" --query "Volumes[0].VolumeId" --output text)
fi
...
if [ "$VOLUME_ID" != "None" ] && [ ! -z "$VOLUME_ID" ]; then
  ...
  aws ec2 create-tags --region $REGION --resources $SNAPSHOT_ID --tags Key=Name,Value="spot-interruption-snapshot-${ENVIRONMENT}" Key=Environment,Value=${ENVIRONMENT}
